...
#import "AdjustmentTrainingCell.h"
#import "CourseActionTableViewCell.h"
...
char * const XYRollViewIsEditingKey = "XYRollViewIsEditingKey";
...
//    }
    ...
    if (longPress.state == UIGestureRecognizerStateBegan) {
        ...
        //如果此cell是课程修改cell，并且当前cell的动作已经完成，则不可拖动
...
- (void)_updateRollingDataSource {
    ...
    if ([self.rollingTempArray xy_isArrayInChildElement]) {
        //是嵌套数组
        if ([self transformSectionOfIndexPath:self.lastRollIndexPath] == [self transformSectionOfIndexPath:self.xy_newRollIndexPath]) {
            //在同一个section内
            //获得当前选中的模块
            ActionModuleModel *moduleModel = self.rollingTempArray[[self transformSectionOfIndexPath:self.lastRollIndexPath]];
            //将动作数组的list元素换位置
//            NSMutableArray *tempMutableArray = [NSMutableArray arrayWithArray:moduleModel.list];
//            [tempMutableArray exchangeObjectFromIndex:self.lastRollIndexPath.row toIndex:self.xy_newRollIndexPath.row];
//            moduleModel.list = tempMutableArray;
            //学员智能课程的动作，可能有副动作，所以需要对分组动作进行排序
            //只能交换单组分组动作
            NSMutableArray *tempMutableArray = [NSMutableArray arrayWithArray:moduleModel.showActionArray];
            [tempMutableArray exchangeObjectFromIndex:self.lastRollIndexPath.row toIndex:self.xy_newRollIndexPath.row];
            [moduleModel exchangeGroupArrayQueue:tempMutableArray];
            
//            [self.rollingTempArray[[self transformSectionOfIndexPath:self.lastRollIndexPath]] exchangeObjectFromIndex:self.lastRollIndexPath.row toIndex:self.xy_newRollIndexPath.row];
        }
        else {
            //不在同一个section内
            // 容错处理：当外界的数组实际类型不是NSMutableArray时，将其转换为NSMutableArray
            id originalObj = self.rollingTempArray[[self transformSectionOfIndexPath:self.lastRollIndexPath]][self.lastRollIndexPath.item];
            [self.rollingTempArray[[self transformSectionOfIndexPath:self.xy_newRollIndexPath]] insertObject:originalObj atIndex:self.xy_newRollIndexPath.item];
            [self.rollingTempArray[[self transformSectionOfIndexPath:self.lastRollIndexPath]] removeObjectAtIndex:self.lastRollIndexPath.item];
        }
    }
    else {
        //不是嵌套数组
...
//自定义section，因为UITableView的组数是从1开始的
- (NSInteger)transformSectionOfIndexPath:(NSIndexPath *)indexPath {
    //如果是排课页面的cell，因为UITableView的组数是从1开始的，所以默认组数减一
    UIView *cell = [self cellForIndexPath:indexPath];
    if ([cell isKindOfClass:[AdjustmentTrainingCell class]]) {
...
- (void)cellSelectedAtIndexPath:(NSIndexPath *)indexPath {
    ...
    UIView *cell = [self cellForIndexPath:indexPath];
    //自定义，当移动cell时，退出cell的编辑模式
    if ([cell isKindOfClass:[AdjustmentTrainingCell class]] || [cell isKindOfClass:[CourseActionTableViewCell class]]) {
        [((AdjustmentTrainingCell *)cell) closeCellWithoutAnimation];
        [((CourseActionTableViewCell *)cell) closeCellWithoutAnimation];
    }
...
//    NSLog(@"lastRollIndexPath:%@-------newIndexPath:%@", self.lastRollIndexPath, newIndexPath);
    ...
    //在cell没有停止拖动时，一直都隐藏cell
    [self cellForIndexPath:self.lastRollIndexPath].hidden = YES;
}
...
//是否允许编辑属性
- (void)setCanMoveCell:(BOOL)canMoveCell {

    self.xy_longPress.enabled = canMoveCell;
    
    objc_setAssociatedObject(self, XYRollViewIsEditingKey, @(canMoveCell), OBJC_ASSOCIATION_RETAIN_NONATOMIC);
}

- (BOOL)canMoveCell {
    return [objc_getAssociatedObject(self, XYRollViewIsEditingKey) boolValue];
}
...
- (UIImageView *)screenshotViewWithShadowOpacity:(CGFloat)shadowOpacity shadowColor:(UIColor *)shadowColor {
    ...
    if ([self isKindOfClass:[UITableViewCell class]]) {
        ...
        UIView *showView;
        if ([cell isKindOfClass:[AdjustmentTrainingCell class]]) {
            UIView *contentView = [cell.contentView viewWithTag:screenShotViewTag];
            showView = [contentView viewWithTag:screenShotContentViewTag];
        } else if ([cell isKindOfClass:[CourseActionTableViewCell class]]) {
            showView = [cell.contentView viewWithTag:screenShotViewTag];
        } else {
            showView = cell.contentView;
        }

        if (showView) {
            // 开启图形上下文
            UIGraphicsBeginImageContextWithOptions(showView.bounds.size, NO, 0);
            ...
            screenshotImageView.layer.shadowOffset = CGSizeMake(-3.0, 0.0);
            screenshotImageView.layer.shadowRadius = 3.0;
...
- (BOOL)xy_isArrayInChildElement {
    NSInteger founIdx = [self indexOfObjectPassingTest:^BOOL(id  _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
         ...
         */
        //修改判断是否是嵌套数组的条件
        return [obj isKindOfClass:[ActionModuleModel class]];
    }];
