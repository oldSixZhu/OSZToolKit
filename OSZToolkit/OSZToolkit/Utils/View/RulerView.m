...
@property (nonatomic, strong) UICollectionView *rulerCollectionView;
...
- (void)layoutViews {
    ...
    if (activeSelectionNumber >= 0) {
        if (self.isDecimal) {
            ...
            if ([Tools isPureInt:defaultValue]) {
...
- (void)correctionDeviation:(double)offset {
    ...
    if (kDirectionHorizontal) {
        self.indicatorView.v_x += deviation;
    } else {
        self.indicatorView.v_y += deviation;
    }
}
