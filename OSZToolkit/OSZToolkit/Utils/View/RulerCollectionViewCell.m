...
- (void)layoutSubviews {
    ...
    if (self.index % 10 == 0) {
        ...
        //字体
        CGSize size = [text boundingRectWithSize:CGSizeMake(SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: self.numberFont} context:nil].size;
...
- (void)makeCellSelect {
    ...
    CGSize size = [text boundingRectWithSize:CGSizeMake(SCREEN_WIDTH, CGFLOAT_MAX) options:NSStringDrawingUsesLineFragmentOrigin attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Semibold" size:kFloat(18)]} context:nil].size;
    ...
    self.selectTextLayer.string = [[NSAttributedString alloc] initWithString:text attributes:@{NSFontAttributeName: [UIFont fontWithName:@"PingFangSC-Semibold" size:kFloat(18)], NSForegroundColorAttributeName: UIColorFromHex(0x20C6BA)}];
